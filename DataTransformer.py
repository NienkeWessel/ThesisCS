...
class FeatureDataTransformer(DataTransformer):
    def __init__(self, words, labels, comparison_pw) -> None:
        ...
        if ngrams: 
            ...
            ngram_features = vectorizer.fit_transform(words)
    ...
    def counts(self, word, comparison_pw, levenshtein=False):
        ...
        if levenshtein: 
            ...
            return [length, alpha_lower, alpha_lower/length, alpha_upper, alpha_upper/length, numeric, numeric/length, special, special/length, char_sets, self.count_non_repeating(s), lev_d]
        else:
            return [length, alpha_lower, alpha_lower/length, alpha_upper, alpha_upper/length, numeric, numeric/length, special, special/length, char_sets, self.count_non_repeating(s)]
